...
replace_empty_values_with_text <- function(dataname, colname, text){
  ...
  if(colname %in% colnames(data)){
    ...
    stop(paste("Column", colname ,"not found in data table"))
  }
}

replace_empty_values_with_text_and_add_text_based_on_values_from_column <- function(dataname, colname, text, colname_for_add_text){
  
  data <- eval(parse(text = dataname))
  
  if(colname %in% colnames(data)){
    to_be_replaced <- paste0(dataname, "$",colname,"[is.na(", 
                             dataname, "$", colname, ")] <-")
    
    replacement <- paste0("paste0('", text, " (',",
                          dataname, "$",colname_for_add_text, "[is.na(", 
                          dataname, "$", colname,  ")],')')")
